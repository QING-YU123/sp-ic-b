...
export class PostCommentController {
  ...
  async query(@Body() postCommentQueryDto: PostCommentQueryDto) {
    ...
    if (!NumberTool.isIntegerInRange(postCommentQueryDto.body.pageSize, 1, 100)) return Result.fail(MsgConst.pageSizeE);
    if (!NumberTool.isInteger(postCommentQueryDto.body.pageIndex)) return Result.fail(MsgConst.pageIndexE);
